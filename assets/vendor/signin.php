...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $mysqli->prepare("SELECT id, password FROM users WHERE username = ?"); // Получаем id и password
    if ($stmt === false) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            // Устанавливаем флаг админа на основе имени пользователя (как было изначально)
            if ($username === 'admin') {
